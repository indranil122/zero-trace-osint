...
const Dialog = ({ open, onOpenChange, children }) => {
  if (!open) return null
  return <>{React.Children.map(children, (c) => React.isValidElement(c) ? React.cloneElement(c, { onOpenChange }) : c)}</>
}

const DialogOverlay = React.forwardRef(({ className, onOpenChange, ...props }, ref) => (
  <div ref={ref} className={cn('fixed inset-0 z-50 bg-black/40 backdrop-blur-sm', className)} onClick={() => onOpenChange?.(false)} {...props} />
))
